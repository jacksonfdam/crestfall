...
/** Grounded and still, both axes loose on the board beside him. */
const RIDE_DOWN: Pose<RiderBone> = {
  spine: [0.12, 0.18, 0], chest: [0.08, 0.08, 0], head: [0.3, 0.35, 0],
  armL: [-0.35, 0, 1.15], forearmL: [0.3, 0, 0.3],
  armR: [-0.4, 0, -1.0], forearmR: [0.25, 0, 0],
  handL: [0, 0, 0.2], handR: [-1.3, 0, 0],
  legL: [0.3, 0, 0.35], shinL: [-0.45, 0, 0],
  legR: [0.1, 0, -0.4], shinR: [-0.7, 0, 0],
  weapon: [-1.5, 0.4, 0],
  mount: [0, 0, 0], mountHead: [0, 0, 0],
};

/**
 * The horse rears, throws him, and bolts to its own right; the rider follows an
 * arc down onto the board on the other side.
 *
 * The rider's hips hang off the mount bone, so unhorsing means moving the horse
 * out from under him: the rider's desired offset from the rig root is converted
 * back into mount-local space each frame (yaw inverted exactly; the mount's
 * residual pitch is deliberately left in, and reads as the tumble).
 */
function defeatBerserkr(
  ...
  const throwOff = easeOutCubic(phase(k, 0.22, 0.62));
  const land = smooth(phase(k, 0.55, 0.8));
  const bolt = easeInCubic(phase(k, 0.34, 1));

  blendPose(v, RIDE_GUARD, RIDE_THROWN, smooth(phase(k, 0.12, 0.5)));
  ...
  if (mount) {
    ...
    if (hips) {
      // Thrown out of the saddle, over the horse's shoulder, down onto the
      // board on its other side — an arc in root space, off the saddle.
      const sit = r.mountP[1] + r.mountHipsP[1];
      seatRider(
        v,
        lerp(0, -0.62 - r.mountHipsP[0], throwOff),
        lerp(0, s.y + 0.3 - sit, throwOff) + 0.34 * bump(throwOff),
        lerp(0, 0.22 - r.mountHipsP[2], throwOff),
      );
      hips.rotation.set(
        1.15 * land - 0.4 * throwOff * (1 - land),
        0.3 * land,
        0.25 * land,
      );
    }
